...
from .tg.tg import start_Scheduler_img,start_Scheduler_text
import asyncio
...
def spam_pro(request):
    ...
    if request.user.is_authenticated:
        if request.method == "POST":
            ...
            data = request.POST
            print(f'data===>{data["date_send"]}<><><><> {data["time_send"]}')
            post = form.save(commit=False)
            post.user = request.user
            post.save()
            try:
                ...
                if len(str(post.file))>0:
                    ...
                    phon=data["gender"]
                    username=data["contact"]
                    t1 = Thread(target=start_Scheduler_text, args=(tepm_date, text, phon, username,))
                    t1.start()
